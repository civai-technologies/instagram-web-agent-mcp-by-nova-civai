...
**Keywords:** mcp-server, model-context-protocol, nova-by-civai, remote-mcp, streamable-http, civai, cursor, claude-desktop, instagram, social-media, dm
...
| Resource | URL |
|----------|-----|
| MCP endpoint | https://nova.civai.co/mcp/agents/instagram-web-agent |
| Agent page | https://nova.civai.co/agent/instagram-web-agent |
| API keys | https://nova.civai.co/api-keys |
| Official Registry | `co.civai.nova/instagram-web-agent` |
...
Local test: copy this folder to `~/.cursor/plugins/local/instagram-web-agent-by-nova`, reload the window, set `NOVA_API_KEY`, verify MCP tools load.
...
```bash
claude mcp add --transport http instagram-web-agent-by-nova \
...
**Nova by CIVAI** is a platform for running, building, and deploying AI agents. This MCP server exposes **Instagram Agent** tools to external MCP hosts without sharing upstream credentials.
